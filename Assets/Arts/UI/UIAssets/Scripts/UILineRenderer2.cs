...
public class UILineRenderer2 : MaskableGraphic
{
    ...
    public bool SetDotAmount(int amount) 
    {
        try {
            ...
            if (transform.childCount <= 0) throw new Exception("적합한 점 프리팹 대상이 없습니다. 해당 객체에 자식으로 추가해주세요.");
            ...
            Transform hLineContainer = transform.parent.GetChild(transform.GetSiblingIndex()-1).GetChild(3);

            if (hLineContainer == null) throw new Exception("UILineRenderer3 : hLineContainer를 찾지 못했습니다.");
            if (hLineContainer.childCount < 2) throw new Exception("UILineRenderer3 :  x좌표 범위를 계산하기 위해 충분한 hLineContainer의 자식을 찾지 못했습니다.");

            dotRangeOfX = (
                hLineContainer.GetChild(0).position.x,
                hLineContainer.GetChild(hLineContainer.childCount - 1).position.x);
            //Debug.Log($"dotRangeOfX[{dotRangeOfX.min}:{dotRangeOfX.max}]");
